...
const jobApiService = {
  ...
  // PERMANENT DELETE - Removes job from database
  deleteJob: async (id) => {
    try {
      const response = await fetch(`${API_BASE_URL}/jobs/${id}`, {
        method: 'DELETE',
  ...
  // Fetch applicant count for a single job
  getApplicantCount: async (jobId) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/candidate-profile-job-application?job_id=${jobId}`
  ...
  // Fetch applicant counts for multiple jobs in parallel
  getApplicantCounts: async (jobIds) => {
    try {
      const promises = jobIds.map((id) =>
        jobApiService.getApplicantCount(id).catch(() => 0)
...
export default function JobListings() {
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      // Filter out deleted jobs if they have a deleted flag
      const transformedJobs = jobsData
        .filter(job => job.is_deleted !== true && job.job_status !== 'deleted')
        .map(job => ({
          id: job.id || job._id || Math.random().toString(),
          title: job.title || job.job_title || 'Untitled',
          company: job.company_name || job.Company?.company_name || job.company || 'N/A',
          location: job.location_name || job.location || job.Location?.name || 'N/A',
          type: job.posting_type || job.job_type || 'Full-time',
          applications: job.applications_count || job.apps || 0,
          status: job.is_status === true || job.is_status === 1 || job.status === 'active' ? 'active' 
            ...
            : 'draft',
          created_at: job.created_at || job.createdAt || job.posted_date || new Date().toISOString(),
          is_trending: job.is_trending || false,
          reference_code: job.reference_code || job.code,
          views: job.views || job.job_views || 0,
        }));
  ...
  // Confirm delete - PERMANENTLY removes the job
  const handleConfirmDelete = async () => {
    if (!jobToDelete) return;
    
    const { id, title } = jobToDelete;
    setDeletingId(id);
    setShowDeleteModal(false);
    
    try {
      await jobApiService.deleteJob(id);
      showSuccess(`"${title}" permanently deleted successfully!`);
      // Refresh the job list after deletion
      await fetchJobs();
    } catch (error) {
      console.error('Delete error:', error);
      showError(`Failed to delete "${title}". ${error.message || 'Please try again.'}`);
    } finally {
      setDeletingId(null);
      setJobToDelete(null);
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900">{trendingJobs}</p>
                <p className="text-xs text-gray-500">Trending</p>
              </div>
            </div>
          </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        className={`p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors ${
                          deletingId === job.id ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-xl border border-gray-200 px-4 py-3 shadow-sm">
                <div className="text-sm text-gray-500">
                  ...
                  of <span className="font-medium text-gray-700">{totalItems}</span> jobs
                </div>
        ...
        {filteredJobs.length > 0 && (
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4 text-sm text-gray-500">
            ...
            <div className="flex items-center gap-4">
              <span>👁️ {jobs.reduce((sum, j) => sum + (j.views || 0), 0)} job views</span>
              <span>📝 {totalApplications} Total applications</span>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-6 animate-slide-in">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2 bg-red-50 rounded-full">
                <TbTrash className="text-red-500" size={24} />
              </div>
            ...
            <p className="text-sm text-red-500 mb-6">
              ⚠️ This action cannot be undone. All applications and data associated with this job will be permanently removed.
            </p>
